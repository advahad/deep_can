...
def create_one_label(df):
    value_counts_percentage = df['road_type'].value_counts(normalize=True, dropna=False) * 100
    ...
    if not there_is_nan and number_of_classes == 1:
        ...
        return None
    elif number_of_classes == 1 and there_is_nan:
        if nan_percent < 30:
            df[[OLD_TARGET]] = df[OLD_TARGET].fillna(method='ffill')
            df[[OLD_TARGET]] = df[OLD_TARGET].fillna(method='bfill')
            return df
        else:
            return None
...
def create_windows(df, seq_size, window, start_seq_idx, windowed_data_dict):
...
def create_europen_sequences(labeled_data_path, seq_size, step_size, results_dir, cols_to_drop):
...
def create_european_sequences_separate_files_results(labeled_data_path, seq_size, step_size, results_dir, cols_to_drop):
    # results_file_name = results_dir + 'seq_size_' + str(seq_size) + '_step_' + str(step_size) + '.csv'
    sessions_files = os.listdir(labeled_data_path)
...
def create_european_sequences_separate_files_per_series_results(labeled_data_path, seq_size, step_size, results_dir, cols_to_drop):
    # results_file_name = results_dir + 'seq_size_' + str(seq_size) + '_step_' + str(step_size) + '.csv'
    sessions_files = os.listdir(labeled_data_path)
