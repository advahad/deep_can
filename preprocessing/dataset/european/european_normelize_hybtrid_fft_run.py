...
ENCODER_DIR_PATH = '../results/raw_preprocessing/windows/encoder/'
ONE_FILE_PATH = '../results/raw_preprocessing/windows/seq_100_step_100/samples_one_file/samples_one_file_merged_labels.csv'
...
meta_cols = list(relevant_cols.loc[(relevant_cols['col_type'] == 'meta')]['col_name'].values)
meta_cols = ['series_num', 'vid', 'sid', 'fid_x', 't_x']
...
def transform(src_data_dir_path, results_preprocessed_data_dir_path, encoder, categorical_features,
    ...
    for idx, file in enumerate(samples_files):
        ...
        if should_encode_cat == True:
            ...
            categorical_df = pd.DataFrame(sample_categorical, columns=encoder.get_feature_names(['280']))
        else:
