...
from model.european.baseline import baseline_main as baseline_util
import pandas as pd
...
general_conf_path = '../config/general_config_new_run.json'
...
output_dir = 'final_output/european/seed_{}/'.format(seed)
...
def create_data_for_meta_learner(label_encoder, y_preds_nn_path, y_preds_nn_basic_path, y_preds_ts_xgboost_path,
    ...
    top_features = \
        baseline_util.get_feature_importance(
    ...
    # concat all feature
    merged_preds_X = pd.concat(
        [embedding_features, xgboost_features, y_pred_proba_NN_AE, y_pred_proba_NN_basic, y_pred_proba_tsfresh], axis=1)
...
def train_meta_learning(label_encoder, y_train_preds_nn_path, y_train_preds_nn_basic_path,
    ...
    if should_train_tpot == True:
        tpot_classifier.find_best_pipline_tpot(merged_preds_X, y_correct_encoded, tpot_model_path)

    exported_pipeline = LogisticRegression(
        class_weight={'motorway': 1, 'secondary': 1, 'residential': 0.6},
        max_iter=1000)
...
def generate_predictions_data_for_meta_learner(set_type):
    if set_type == 'val':
        ...
        baseline_util.create_aggregate_features(samples_dir_path=paths_conf_obj.ts_fresh_samples_path,
                                                ...
                                                kind_params_path='../../../output/seq_200_step_25/king_params/' + general_conf_obj.tsfresh_offline.kind_params_file_name,
...
if __name__ == '__main__':
    ...
    if general_conf_obj.running.train.meta_learner:
        ...
        nn_set_dir, nn_basic_set_dir, tsfresh_set_dir, ae_set_dir = generate_predictions_data_for_meta_learner(
            proba_set_for_meta_learner)
        # nn_set_dir = output_handler.val_sets_dir_nn
        # nn_basic_set_dir = output_handler.val_sets_dir_nn_basic
        # tsfresh_set_dir = output_handler.val_sets_dir_xgboost_online
        # ae_set_dir = output_handler.val_sets_dir_ae
