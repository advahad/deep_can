...
from evaluation import confusion_matrix_util
from model.common.helpers import run_helper
from preprocessing.dataset.SHRP2 import shrp2_raw_preprocessing_NN as preprocess_util
from util import pickle_util
from sklearn.metrics import balanced_accuracy_score


# return output_handler, general_conf_obj, paths_conf_obj
...
def extract_baseline_features(type, data_path, timeseries_extracted_df_path, y_path, kind_to_fc_parameters_path,
    ...
    meta_data = pd.read_csv(paths_conf_obj.meta_data_dir_path + META_DATA_FILE_NAME)
    #
    ...
    if save_meta: # for future partitioning
...
def select_baseline_features(X_train_path, y_train_path, kind_to_fc_parameters_path, selected_features_names_path,
    ...
    print('shape of selected features: {}'.format(selected_features_df.shape))
...
def generate_train_test_sets(extracted_df, existing_partitioning, partitioning_type,                             x_train_path, x_test_path, x_val_path,
...
SORT_COL = 'vtti.timestamp'
# DROP_COLS = [target_col, 'sid', 'vid', 't_x']
DROP_COLS = [target_col, 'sub_sid', 'sample_file_name', "vtti.engine_rpm_instant", "vtti.cruise_state", "vtti.pedal_brake_state"]
DROP_COLS = [target_col, 'sub_sid', 'sample_file_name', "vtti.engine_rpm_instant", "vtti.cruise_state", "vtti.pedal_brake_state"]

# offline configuration
STEP_OFFLINE_TRAIN = general_conf_obj.tsfresh_offline.step
STEP_OFFLINE_TRAIN = paths_conf_obj.step_size
MAIN_DIR = '../../../data_SHRP2/windows/seq_{}_step_{}/'.format(SEQ, paths_conf_obj.step_size)
META_DATA_DIR_PATH = '{}/meta_merged_labels_3_cls/'. \
    format(MAIN_DIR, SEQ, STEP_OFFLINE_TRAIN)
...
if __name__ == '__main__':
    ...
    # output_handler, general_conf_obj, paths_conf_obj = init_conf()
    # global_seeds.set_seeds(31)
    ...
    output_handler = run_helper.OutputDirHandler(paths_conf_obj, general_conf_obj, 'output_shrp_baseline/')
    ...
    SORT_COL = 'vtti.timestamp'
    # DROP_COLS = [target_col, 'sid', 'vid', 't_x']
    # DROP_COLS = [target_col, 'sub_sid', 'sample_file_name']
    # DROP_COLS = [target_col, 'sid', 'vid', 'sub_sid', 'sample_file_name', general_conf_obj.partitioning.type]
    ...
    OFFLINE_TRAINING_ONE_FILE_DATA_PATH = \
        '{}samples_one_file_extended_union/samples_one_file_dict.csv'.format(MAIN_DIR, SEQ, STEP_OFFLINE_TRAIN)
    EXTRACTED_FEATURES_DF_PATH = '{}features_filtered_direct_european_seq_{}_step_{}.csv'.format(
        output_handler.output_dir_data_xgboost_offline, SEQ, STEP_OFFLINE_TRAIN)
    ...
    # kind = pickle_util.load_obj(output_dir_data_tsfresh_offlline + general_conf_obj.tsfresh_offline.kind_params_file_name)

    # kind = pickle_util.load_obj(output_dir_data_tsfresh_offlline + general_conf_obj.tsfresh_offline.kind_params_file_name)
    print('kind params will be saved in: {}'.format(output_dir_data_tsfresh_offlline + general_conf_obj.tsfresh_offline.kind_params_file_name))
    ...
    predict(model_path=output_dir_data_tsfresh_offlline + general_conf_obj.tsfresh_offline.xgboost_model_name,
            ...
            y_pred_proba_path=test_sets_dir_tsfresh_online + general_conf_obj.tsfresh_online.y_pred_proba_file_name)
