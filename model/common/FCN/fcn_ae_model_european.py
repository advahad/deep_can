...
class FcnAeSingle:
    def __init__(self, seq_size=None, features_size=None, num_of_classes=None, model_path=None, use_generator=False,
                 ...
                 initialize_weights_with_model=False, es_patience=100, concat_ae=False, input_shape_ae=None, ae_best_model_path=None):
    ...
    def make_model_concat_ae(self, seq_size, features_size, num_of_classes, features_size_ae):
        ...
        conv1 = keras.layers.Conv1D(filters=10, kernel_size=8, padding='same')(input_layer)
        conv1 = keras.layers.BatchNormalization()(conv1)
        conv1 = keras.layers.Activation(activation='relu')(conv1)

        conv2 = keras.layers.Conv1D(filters=10, kernel_size=5, padding='same')(conv1)
        conv2 = keras.layers.BatchNormalization()(conv2)
        conv2 = keras.layers.Activation('relu')(conv2)

        conv3 = keras.layers.Conv1D(10, kernel_size=3, padding='same')(conv2)
        conv3 = keras.layers.BatchNormalization()(conv3)
        conv3 = keras.layers.Activation('relu')(conv3)

        gap_layer = keras.layers.GlobalMaxPool1D()(conv3)
        gap_layer = keras.layers.GlobalAveragePooling1D()(conv3)

        input_layer_ae_embedding_ts = keras.layers.Input((features_size_ae,))
        dense_ae = keras.layers.Dense(10, activation="relu", kernel_regularizer=keras.regularizers.l2(0.001))(input_layer_ae_embedding_ts)
    ...
    def train_fcn_ae(self, x_train, y_train_encoded, x_val, y_val_encoded, x_test, y_test_encoded, best_model_path, epochs,
        ...
        hist = self.model.fit([x_train, x_train_ae_representation], y_train_encoded, validation_data=([x_val, x_val_ae_representation], y_val_encoded),
    ...
    def evaluate_trips(self, y_test_path, y_pred_path, y_pred_proba_path, class_names, target, save_plots_path_prefix):
        ...
        evaluate_test.evaluate_by_files(y_test_path, y_pred_path, y_pred_proba_path, class_names, target, save_plots_path_prefix)
