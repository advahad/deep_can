...
def split_data_by_group(data_df, target, seq_len, should_reshape=True, seed=5):
    ...
    test_size = 10000
    val_size = 10000
    # return X_train[0:train_size, :, :], X_test[0:test_size, :, :], X_val[0:val_size, :, :], y_train[0:train_size], y_test[0:test_size], \
    #        y_val[0:val_size], file_id_event_id_dict
    return X_train, X_test, X_val, y_train, y_test, y_val, file_id_event_id_dict
    # return X_train, X_train, X_train, y_train, y_train, y_train, file_id_event_id_dict
...
class BaseDataContainer:
    def __init__(self, csv_path, target_column, seq_len, remove_cols=None, is_LSTM=True, should_scale=True,
        ...
        all_labels = data[target_column].unique()
        for label in all_labels:
            size = original_data[original_data['aug.road_type'] == label].shape[0]
            print("size of class {} is {}".format(label, size))
        ...
        if should_scale:
            ...
            # override columns that shouldn't scale
